...
var IMG_URL = ASSET_BASE + 'DSniang1.png'
var GIF_URL = ASSET_BASE + 'rua.gif'
// 挂件配置持久化：改存 localStorage（由 index.js 的运行时桥代理）。
...
function showCostBubble(amount, estimated) {
  ...
  // usage 被中转剥掉时走估算兜底：标签与金额都带"≈"，与精确结算明确区分
  gifEl.style.display = 'none'
  gifEl.style.opacity = ''
  labelEl.style.display = ''
  labelEl.className = 'dshwv-label'
...
function viewport() {
  // 移动端优先用 visualViewport：地址栏收展/软键盘引起的是它的变化，
  // innerHeight 在部分浏览器上滞后或包含键盘区域
...
// 软键盘判定：visualViewport 比布局视口矮 25% 以上视为键盘弹出。
// 键盘期间 settle 全部跳过（鲸鱼保持原位），收起后由 resize 重新贴边
function keyboardIsUp() {
  var vv = window.visualViewport
  if (!vv || !window.innerHeight) return false
  return (window.innerHeight - vv.height) / window.innerHeight > 0.25
}
// 安全区（刘海屏/底部横条）：CSS 变量桥接 env()，JS 侧可读。
// 所有贴边计算叠加 safe 距离；锚点存储净距离（恢复时重新叠加，与 scrollGap 同模式）
var safeInsets = { b: 0, r: 0 }
...
function fmt(balance, currency) {
  // 空值安全：null/undefined 经 Number() 会变 0，导致"无余额"误显 ¥ 0.00
  if (balance === null || balance === undefined) {
    return currency === 'CNY' ? '¥ --' : '--' + (currency ? ' ' + currency : '')
  }
...
function render() {
  ...
  } else if (state.status === 'nokey') {
    // 未配置 Key：余额不可查，但消耗统计照常显示
    amount = '--'
    hint = '今日已用 ' + (state.todayUsage !== null && state.todayUsage !== undefined ? fmt(state.todayUsage, 'CNY') : '--')
...
function settle() {
  ...
  if (state.h === 'right') {
    state.left = Math.max(0, vp.w - w - state.hOff - rightGap() - safeInsets.r)
  } else if (state.h === 'left') {
    state.left = state.hOff
  } else {
    state.left = clamp(state.left, 0, Math.max(0, vp.w - w - rightGap() - safeInsets.r))
  }  if (state.v === 'bottom') {
    state.top = Math.max(0, vp.h - h - state.vOff - safeInsets.b)
  } else if (state.v === 'top') {
    state.top = state.vOff
  } else {
    state.top = clamp(state.top, 0, Math.max(0, vp.h - h - safeInsets.b))
  }
  express()
}
...
function refresh(manual) {
  if (busy) return
  var rt = window.__dafyRuntime
  if (!rt || typeof rt.getBalance !== 'function') {
    state.status = 'error'
    state.message = '运行时未就绪'
    render()
    return
  }
  busy = true
  if (animDelayTimer) { clearTimeout(animDelayTimer); animDelayTimer = null }
  var auto = updateAutoDim()
  // 非官渠且非手动点击：不发起任何余额查询，仅刷新消耗统计
  if (!manual && !auto) {
...
function saveConfig() {
  try {
    ...
    // 存净距离：剥离安全区偏移，恢复时按当前设备重新叠加（跨设备/旋转后仍正确）
    if (hAnchor === 'right') hDist = Math.max(0, hDist - safeInsets.r)
...
function applySoundSet() {
  try {
    pressAudio = new Audio(ASSET_BASE + (soundSet === 'fx1' ? 'D1.mp3' : 'Ya1.mp3'))
    pressAudio.preload = 'auto'
    pressAudio.volume = soundVol
    releaseAudio = new Audio(ASSET_BASE + (soundSet === 'fx1' ? 'D2.mp3' : 'Ya2.mp3'))
...
function snapCheck() {
  ...
  if (centerX < vp.w / 4) {
    ...
    state.h = 'right'
    state.hOff = 0
    left = vp.w - w - rightGap() - safeInsets.r
    moved = true
  } else {
    state.h = null
    state.hOff = left
...
// —— 触屏支持 ——
// touchstart 命中鲸鱼时阻止默认行为：阻止页面滚动（pointerdown 的 preventDefault
// 阻止不了滚动）与 iOS 长按系统弹窗；拖拽/点击仍由 pointer 事件流处理
function onDocTouchStart(e) {
  if (!e.touches || !e.touches.length) return
  if (!isWhaleHit(e.touches[0])) return
  try { e.preventDefault() } catch (err) {}
}
document.addEventListener('touchstart', onDocTouchStart, { passive: false, capture: true })
// 长按弹菜单（触屏的"右键"）：按下 500ms 未移动即打开，触发后本次手势不再拖拽/弹泡
  ...
  if (drag.pointerType === 'touch') {
    ...
    longPressTimer = setTimeout(function () {
      ...
      if (drag && drag.active && !drag.moved) {
        drag.suppressed = true // 打开菜单后：本次手势不再移动鲸鱼，抬起也不弹泡泡
        toggleMenu()
      }
    }, 500)
  }
...
function onDocPointerMove(e) {
  if (!drag || !drag.active || drag.suppressed) return
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
  // 点击容差按指针类型分流：触屏抖动大（10px），鼠标精准（3px）
  var thr = drag.pointerType === 'touch' ? 100 : CLICK_SQ
  if (dx * dx + dy * dy >= thr) {
    drag.moved = true
    clearLongPress() // 已开始拖拽：取消长按菜单
  }
...
function onDocPointerMoveCursor(e) {
  // 仅精确指针（鼠标/笔）做 hover 逻辑：触摸滑动会高频触发 pointermove，
  // elementFromPoint 白算且触屏没有 hover 概念
...
function applyAnchorPos() {
  try {
    var a = JSON.parse(localStorage.getItem('dafy-pos') || 'null')
    ...
    var w = root.offsetWidth || root.getBoundingClientRect().width || 0
    var h = root.offsetHeight || root.getBoundingClientRect().height || 0
    // 与加载恢复一致：锚点存净距离，右/底锚点按当前避让开关与安全区叠加
    var effectiveRightDist = a.hAnchor === 'right' ? a.hDist + (scrollGapOn ? rightGap() : 0) + safeInsets.r : a.hDist
    var l = a.hAnchor === 'left' ? a.hDist : vp.w - effectiveRightDist - w
    var t = a.vAnchor === 'top' ? a.vDist : vp.h - a.vDist - h - safeInsets.b
...
var settleDebounce = null
function onViewportResize() {
  // 防抖：移动端地址栏收展/旋转会连续触发 resize，150ms 合并一次
  if (settleDebounce) clearTimeout(settleDebounce)
  settleDebounce = setTimeout(function () {
    settleDebounce = null
    refreshSafeInsets() // 旋转后安全区可能变化
    // 软键盘弹出（视口骤降）：鲸鱼保持原位；收起时 resize 会再次触发、重新贴边
...
refreshSafeInsets() // 安全区（刘海/底部横条）：settle/锚点计算要用
...
applySoundSet()
setupHitTest()
// 挂件配置加载：从 localStorage（运行时桥）读取，同步生效
;(function loadConfig() {
  var rt0 = window.__dafyRuntime
  ...
  if (d && typeof d.usageMode === 'string') {
    usageMode = d.usageMode === 'engine' ? 'engine' : 'ledger'
    usageSelect.value = usageMode
  }
  ...
  try {
    ...
    if (a && a.v === 2 && (a.hAnchor === 'left' || a.hAnchor === 'right') && typeof a.hDist === 'number' &&
      ...
      // 锚点存的是净距离：右/底锚点按当前避让开关与安全区叠加
      var effectiveRightDist = a.hAnchor === 'right' ? a.hDist + (scrollGapOn ? rightGap() : 0) + safeInsets.r : a.hDist
      var lA = a.hAnchor === 'left' ? a.hDist : vpA.w - effectiveRightDist - wA
      var tA = a.vAnchor === 'top' ? a.vDist : vpA.h - a.vDist - hA - safeInsets.b
...
// —— 每轮对话消耗：订阅运行时桥的回合事件（index.js 截获真实 usage / 估算兜底后推送） ——
var rtTurn = window.__dafyRuntime
if (rtTurn && typeof rtTurn.onTurn === 'function') {
  rtTurn.onTurn(function (turn) {
...
if (rtTurn && typeof rtTurn.onState === 'function') {
  rtTurn.onState(function () {
    refresh(false)
  })
}
})()